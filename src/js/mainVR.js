...
class Main extends AbstractVRApplication {
  ...
  async init () {
    ...
    const deathstar = await this.loadModel('assets/models/deathstar.stl')
    console.log(deathstar)
    // const geometry = new THREE.BoxGeometry(0.15, 0.15, 0.15)

    for (let i = 0; i < 2; i++) {
      const object = new THREE.Mesh(deathstar, new THREE.MeshLambertMaterial({ color: Math.random() * 0xffffff }))

      object.position.x = Math.random() * 10 - 2
      object.position.y = Math.random() * 10 - 2
      object.position.z = Math.random() * 10 - 2
